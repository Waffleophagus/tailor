...
| **Effective Access** | The resolved, concrete reachability between two devices after evaluating all ACL rules, groups, tags, autogroups, destination ports, and protocols. Distinct from **Device Visibility** (which devices appear in your netmap). |
| **Device Visibility** | Which devices appear in a perspective's trimmed netmap per Tailscale [device visibility](https://tailscale.com/docs/concepts/device-visibility): sources can reach (outbound ACL), same user identity, and peers that can reach sources (inbound). Symmetric — if A may connect to B, both appear in each other's netmap. |
...
| **Policy Workbench** | The Tailscale-shaped policy editing surface: POLICY nav (general access rules, SSH, tests, auto-approvers) and DEFINITIONS nav (groups, tags, IP sets, hosts, device posture, node attributes). Primary place to configure ACLs; graph is the preview. |
...
| **Tailor Access Role** | Tailor's request-level authorization classification derived from tsnet `WhoIs` identity and app capabilities. The first roles are `full` and `viewer`. |
| **Tailor App Capability** | The configurable Tailscale app capability used to grant Tailor-specific permissions through the tailnet policy file. By default, Tailor derives it from the tsnet node's MagicDNS name, such as `tailor.triceratops-gecko.ts.net/cap/admin`. |
| **Tailor ACL Service Tag** | The destination device tag `tag:tailor-acl-service` for the Tailor tsnet node's ACL editing service surface. Distinct from any future Tailor visibility or discovery tags. |
| **Tailor ACL Editor Tag** | The explicit source-device tag `tag:tailor-acl-editor`. Requests from devices with this tag get full Tailor access when the policy grants the Tailor App Capability. |
| **Tailor Setup Grant Save** | The narrow first-run Cloud API write that appends Tailor's App Capability grant before normal policy editing is available. It is not a general editing mode and does not grant full access without `WhoIs` capability authorization. |
| **Tailor Bootstrap Session** | A short browser-only fallback authorization state used only when Tailor cannot automatically validate or save the Tailor Setup Grant Save. It is tied to the current browser and `WhoIs` identity, expires after 15 minutes, and does not authorize MCP. |
