...
- **Explicit trust escalation**: Phase 2 requires a deliberate action (pasting an OAuth Client ID/Secret) and is visually distinct from Phase 1.
...
2. Modal prompts for:
  - Tailnet name (auto-filled from LocalAPI)
  - OAuth Client ID
  - OAuth Client Secret
3. Backend exchanges credentials for an Access Token at `api.tailscale.com/api/v2/oauth/token`.
...
| BE-04 | Expose REST endpoints: `GET /api/status`, `GET /api/tailnet`, `POST /api/auth`, `POST /api/validate`, `POST /api/acl` | P0 |
| BE-05 | Store OAuth tokens in memory (not persisted to disk by default) | P0 |
| BE-06 | Implement OAuth Client Credentials flow with `api.tailscale.com/api/v2/oauth/token` | P0 |
| BE-07 | Fetch and cache the tailnet policy file from Cloud API | P0 |
| BE-08 | Resolve ACL rules into effective access edges (group + tag expansion) | P0 |
...
| BE-13 | Support bearer token auth as alternative to OAuth | P1 |
...
- No user impersonation or auth token extraction from Tailscale daemon
...
- An admin can paste an OAuth Client, edit a tag, validate, save, and see the change reflected in the Tailscale admin console within 60 seconds.
